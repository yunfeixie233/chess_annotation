#!/usr/bin/env python3
"""
Evaluate chess SFT model accuracy on best-move prediction.
...
Usage:
    python eval_hf.py --checkpoint /path/to/checkpoint-2000 --skip_base
    python eval_hf.py --checkpoint /path/to/sft/  # evals all checkpoint-* dirs
"""
...
print("Loading torch and transformers (this may take a minute on NFS)...", flush=True)
import torch
...
def extract_move(text):
    ...
    """
    # Try \boxed{move} pattern
    match = re.search(r"\\boxed\{([^}]+)\}", text)
    if match:
        return match.group(1).strip(), True

    # Fallback: try <answer>...</answer> pattern
    ...
    # Last resort: look for "best move is X" pattern
...
@torch.no_grad()
def run_inference(model, tokenizer, samples, max_new_tokens=2048, desc="Evaluating"):
    """Run inference on samples with batch_size=1 and return predictions."""
    model.eval()
    predictions = []

    for i in tqdm(range(len(samples)), desc=desc):
        s = samples[i]
        prompt = build_prompt(s["input"])
        ...
        predicted_move, has_format = extract_move(generated)
        predictions.append({
            "fen": s["input"],
            "best_move": s["best_move"],
            "predicted_move": predicted_move,
            "has_format": has_format,
            "correct": predicted_move == s["best_move"],
...
def save_eval_result(output_dir, model_name, avg_metrics, per_run_metrics, total_time, last_predictions, checkpoint=None):
    ...
    # Save predictions from the last run for inspection
    result["predictions"] = [
...
def eval_single_model(model_path, samples, args, desc, output_dir):
    """Load a model, run eval num_runs times, save averaged results. Returns avg metrics."""
    ...
    tokenizer_source = BASE_MODEL if is_local_checkpoint else model_path
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate chess SFT model")
